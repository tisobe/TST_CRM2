...
c     f77 -C -ftrap=%all,no%inexact runcrm.f ./crmflx_v23/CRMFLX_V23A_EXP.f -o runcrm
...
c     read in the ephemeris data
     ...
     &'/data/mta4/proj/rac/ops/ephem/PE.EPH.gsme_in_Re')
      iv = 0
c     the following line is added feb 11, 2015 (t. isobe)      
      ios = 0
      do while (ios.eq.0)
         iv = iv + 1
         read(1,*,iostat=ios)
     $      t,xgsm,ygsm,zgsm,xgse,ygse,zgse,fyr,mon,d,h,m,s
      ...
      do i = 1,28
        xkp = (i-1)/3.0
        open(unit=2,name='/data/mta4/proj/rac/ops/CRM2/CRM2_p.dat'
     &//fext(i))
        do j = 1,iv
...
c         write(*,*) tg(j),lunit,xkp,xg(j),yg(j),zg(j),idloc,fluxmn,
c    &    flux95,flux50,fluxsd
